...
export const run = (state, { io, domRoot }) => {
  ...
  embed.id = 'embed'
  domRoot.appendChild(embed)
  var div = d3.select("#embed");
  state.svg = div.append("svg") // svg is global
    .attr("width", 500)
    .attr("height", 500);

  // Draw embedding ===
  state.gs = state.svg.selectAll(".b")
    .data(io.data)
    .enter().append("g")
    .attr("class", "u");

  state.cs = state.gs.append("circle")
    .attr("cx", 0)
    .attr("cy", 0)
    .attr("r", 5)
    .attr('stroke-width', 1)
  ...
  if(io.labels.length > 0) {
    state.ts = state.gs.append("text")
  ...
  var zoomListener = d3.behavior.zoom()
    ...
    .center([0,0])
    .on("zoom", () => {
      state.tx = d3.event.translate[0];
      state.ty = d3.event.translate[1];
      state.ss = d3.event.scale;
    });
  zoomListener(state.svg);

  state.T.initDataRaw(io.data)
...
export const update = state => {
  ...
  var Y = state.T.getSolution();
  // move the groups accordingly
  state.gs.attr("transform", function(d, i) {
    const out = "translate(" +
      ((Y[i][0]*20*state.ss + state.tx) + 200) + "," +
      ((Y[i][1]*20*state.ss + state.ty) + 200) + ")";
    return out
  });
}
